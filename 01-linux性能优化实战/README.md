...
* 工具
    ...
    * perf -> 追踪热点进程和函数
        * 可以追踪到短时进程
    * ab -> http性能测试
        * ab -c 5 -t 600 http://192.168.0.10:10000/ -> 执行10min请求
    * pstree -> 追踪进程调用(fork)
        * pstree | grep stress 
...
查看历史监控时，会发现应用在发版的时候，cpu等各项资源有明显的升高，这是正常的，说明
应用启动的时候会消耗较多的系统资源。因此，如果应用因为某些原因反复重启，通常是启动
失败再被定时重启任务拉起，很可能会导致资源紧张，同时由于进程id一直在变，很难通过进程
id的角度去分析问题。

> 问题

* 如何查看不同状态的进程？
* 什么情况下进程id经常发生变化？这类进程引发的问题如何排查？
